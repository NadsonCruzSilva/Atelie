...
public class FXMLProcessosTerceirizacaoDialogController implements Initializable {
    ...
    @FXML
    private ComboBox<OrdemServico> comboBoxOS;
    @FXML
    private ComboBox<FabricaParceira> comboBoxFabrica;
    @FXML
    private ComboBox<TipoServico> comboBoxTipoServico;
    @FXML
    private TextField textFieldQuantidade;
    @FXML
    private DatePicker datePickerEnvio;
    @FXML
    ...
    public void setEncaminhamento(Encaminhamento enc) {
    ...
    public void handleButtonConfirmar() {
        ...
        if (osSelecionada == null) {
            showError("Selecione uma Ordem de Serviço!");
            return;
        }
        if (fabricaSelecionada == null) {
            showError("Selecione uma Fábrica Parceira!");
            return;
        }
        ...
        if (!javafxmvc.utils.ValidadorUtil.isInteiroPositivo(textFieldQuantidade.getText())) {
            showError("Informe uma Quantidade válida (número inteiro maior que zero)!");
            return;
        }
        ...
        if (encDAO.isLimiteOperacionalExcedido(fabricaSelecionada)) {
            ...
            alert.setContentText("A fábrica \"" + fabricaSelecionada.getNome()
                    + "\" já possui 10 ou mais ordens ativas. Não é possível enviar mais peças.");
            alert.showAndWait();
            return;
        }
        ...
        try {
            ...
            encaminhamento.setDataRetornoPrevista(
                    datePickerEnvio.getValue().plusDays(tipoSelecionado.getPrazoEstimadoDias()));
            ...
            boolean insertOk = encDAO.inserir(encaminhamento);
            if (!insertOk) {
                rollback();
                showError("Erro ao inserir encaminhamento no banco de dados.");
                return;
            }

            // Atualizar status da OS
            osSelecionada.setStatus("ENVIADA_FABRICA");
            boolean updateOk = osDAO.alterar(osSelecionada);
            if (!updateOk) {
                rollback();
                showError("Erro ao atualizar o status da Ordem de Serviço.");
                return;
            }

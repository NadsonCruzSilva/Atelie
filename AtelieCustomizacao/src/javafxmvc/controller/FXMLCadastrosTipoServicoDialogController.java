...
public class FXMLCadastrosTipoServicoDialogController implements Initializable {
    ...
    private boolean validarEntradaDeDados() {
        ...
        if (textFieldDescricao.getText() == null || textFieldDescricao.getText().trim().isEmpty()) {
            errorMessage += "Descrição inválida! O campo não pode estar vazio.\n";
        }
        if (!javafxmvc.utils.ValidadorUtil.isValorPositivo(textFieldValor.getText())) {
            errorMessage += "Valor inválido! Deve ser um número maior que zero.\n";
        }
        if (!javafxmvc.utils.ValidadorUtil.isInteiroPositivo(textFieldPrazo.getText())) {
            errorMessage += "Prazo inválido! Deve ser um número inteiro maior que zero.\n";
        }
